  ...
  return (
    <div className="rounded-xl border border-slate-800/60 bg-slate-900/60 p-3">
      ...
      <div className="rounded-lg border border-slate-800 bg-slate-950 p-3">
        <div className={clsx("flex w-full gap-2", isOut ? "justify-end" : "justify-start")}> 
          {props.type === "text" ? (
            <div
              className={clsx(
                "max-w-[70%] rounded-2xl px-3 py-2 text-sm leading-relaxed shadow",
                isOut
                  ? "rounded-br-sm bg-emerald-600/20 text-emerald-100 border border-emerald-500/30"
                  : "rounded-bl-sm bg-slate-800/70 text-slate-100 border border-slate-700"
              )}
            >
