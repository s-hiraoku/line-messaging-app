...
#### Scenario: ImageCropUploader での画像アップロード後
- **WHEN** ユーザーが ImageCropUploader で画像をアップロードする
- **AND** `onImageUploaded` コールバックで画像URLが返される
- **THEN** 「画像エリア分割を有効にする」トグルが操作可能になる
- **AND** カードフォームの `imageUrl` フィールドに画像URLが設定される
...
- **THEN** ImageCropUploader でアップロードされた画像がキャンバスに表示される
- **AND** 定義済みの領域が半透明の矩形で表示される
- **AND** 各領域のテキストラベルが表示される

#### Scenario: ImageCropUploader でアップロードされた画像の使用
- **WHEN** ユーザーが ImageCropUploader で画像をクロップ・ズーム調整してアップロードする
- **THEN** クロップ・ズーム調整後の画像URLがキャンバスで使用される
- **AND** 画像の元サイズ(width, height)が取得される
- **AND** 領域の座標・サイズは画像の元サイズ基準で管理される
