...
### 0. Docker ランタイムを起動（macOS の場合）

このプロジェクトは Docker を使用して PostgreSQL を起動します。macOS では Docker Desktop または Colima が必要です。
...
このアプリケーションは NextAuth v5 を使用した Google OAuth 認証をサポートしています。
...
5. **クライアント ID とシークレットをコピー**
   - 表示されるクライアント ID とクライアントシークレットをコピー
...
3. 「Sign in with Google」をクリックして OAuth フローをテスト

4. 認証成功後、`/dashboard` にリダイレクトされることを確認

### 注意事項

- **保護されたルート**: `/dashboard` 配下のページは認証が必要です
- **セッション管理**: JWT ベースのセッション戦略を使用しています
- **データベース**: OAuth アカウント情報は `Account` テーブルに保存されます
...
リッチメッセージ（imagemap message）は、LINE のサーバーが画像を取得するため、**公開された HTTPS URL が必要**です。`http://localhost:3000`では動作しません。

### 解決方法: Cloudflare Tunnel を使用

すでに Webhook のセットアップで使用している Cloudflare Tunnel を、リッチメッセージのテストにも利用できます。

### 1. Cloudflare Tunnel を起動

Webhook で使用しているトンネルをそのまま利用します：
...
- **本番環境**: Vercel などにデプロイした場合は `NEXT_PUBLIC_BASE_URL` の設定は不要です（自動的に本番 URL が使用されます）。
...
- **Port 5432 already in use**
  - **原因**: 既にポート 5432 が使用されている
  - **確認**: `lsof -i :5432` で使用プロセスを確認
  - **対処**: 既存のプロセスを停止するか、`docker-compose.yml` でポート番号を変更
